...
class Classifier(nn.Module):
    def __init__(self, in_features=740096, num_class=59):
...
class FPA(nn.Module):
    def __init__(self, channels=2048):
        ...
        self.conv_upsample_3 = nn.ConvTranspose2d(channels_mid, channels_mid, kernel_size=3, stride=2, padding=1, bias=False)
...
class GLPM(nn.Module):
    def __init__(self, channels_high, channels_low, upsample=True):
        ...
        self.conv2x1 = nn.Conv2d(channels_low, channels_high, kernel_size=1, padding=0, bias=False)
        self.conv1x1 = nn.Conv2d(channels_high, channels_low, kernel_size=1, padding=0, bias=False)
    ...
    def forward(self, fms_high, fms_low, fm_mask=None):
        ...
        fms_high_gp = self.conv2x1(fms_high_gp)
        fms_high_gp = self.conv1x1(fms_high_gp)
...
class PAN(nn.Module):
    def __init__(self, blocks=[]):
        ...
        self.fpa1=nn.Conv2d(2048,2048,kernel_size=1,stride=1,padding=0)
        ...
        self.gau_block1 = GLPM(channels_blocks[0], channels_blocks[1], upsample=False)
        self.gau_block2 = GLPM(channels_blocks[1], channels_blocks[2])
        self.gau_block3 = GLPM(channels_blocks[2], channels_blocks[3])
        self.gau = [self.gau_block1, self.gau_block2, self.gau_block3]

        self.relu = nn.ReLU(inplace=True)

        self.conv3 = nn.Conv2d(256, 59, kernel_size=1, stride=1, padding=0)
        self.cll = Classifier()
    def forward(self, fms=[]):
        ...
        for i, fm_low in enumerate(fms):
            if i == 0:
                ...
                fm_high = self.fpa1(fm_low)
            else:
                fm_high = self.gau[int(i-1)](fm_high, fm_low)
        fm_high = self.conv3(fm_high)
        fm_high = fm_high.view(fm_high.size(0), -1)
        fm_high = self.cll(fm_high)
        return fm_high
